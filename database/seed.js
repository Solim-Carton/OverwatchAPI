...
const { db, Hero, Role, Map } = require('./setup');
...
async function seedHeroes() {
    ...
    // Create Roles
        ...
        const damage = await Role.create({ name: 'Damage' });
        const support = await Role.create({ name: 'Support' });

        // Create Maps
        const busan = await Map.create({ name: 'Busan' });
        const dorado = await Map.create({ name: 'Dorado' });
        const shambali = await Map.create({ name: 'Shambali Monastery' });
        const Numbani = await Map.create({ name: 'Numbani'});
        const kings = await Map.create({ name: 'Kings Row'});
        const Eichenwalde = await Map.create({ name: 'Eichenwalde'});
        const Nepal = await Map.create({ name: 'Nepal'});
        const Oasis = await Map.create({ name: 'Oasis'});
...
console.log('Database seeded successfully!');
        ...
        // Only close DB if running directly
...
// Export for tests or other scripts
